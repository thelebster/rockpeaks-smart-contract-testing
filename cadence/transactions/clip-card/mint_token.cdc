...
transaction(recipient: Address, nodeId: String) {

    // local variable for storing the minter reference
    let minter: &RockPeaksClipCard.NFTMinter

    prepare(signer: AuthAccount) {

        // borrow a reference to the NFTMinter resource in storage
        self.minter = signer.borrow<&RockPeaksClipCard.NFTMinter>(from: RockPeaksClipCard.MinterStoragePath)
