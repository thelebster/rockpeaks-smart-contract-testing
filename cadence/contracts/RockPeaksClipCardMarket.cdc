...
/*
    This is a simple RockPeaksClipCard initial sale contract for the DApp to use
    in order to list and sell RockPeaksClipCard.
...
pub contract RockPeaksClipCardMarket {
    // SaleOffer events.
    //
    ...
    // SaleOffer
    // A RockPeaksClipCard NFT being offered to sale for a set fee paid in Peakon.
    //
    pub resource SaleOffer: SaleOfferPublicView {
        ...
        // The RockPeaksClipCard NFT ID for sale.
        pub let itemID: UInt64
        ...
        access(self) let sellerItemProvider: Capability<&RockPeaksClipCard.Collection{NonFungibleToken.Provider}>

        // The Peakon vault that will receive that payment if teh sale completes successfully.
        access(self) let sellerPaymentReceiver: Capability<&Peakon.Vault{FungibleToken.Receiver}>

        // Called by a purchaser to accept the sale offer.
        // If they send the correct payment in Peakon, and if the item is still available,
        // the RockPeaksClipCard NFT will be placed in their RockPeaksClipCard.Collection .
        //
        pub fun accept(
            buyerCollection: &RockPeaksClipCard.Collection{NonFungibleToken.Receiver},
            buyerPayment: @FungibleToken.Vault
        ) {
            pre {
                // Price minus 10% commission.
                buyerPayment.balance == self.price - (self.price * 0.1): "payment does not equal offer price"
        ...
        // to transfer the RockPeaksClipCard NFT and the capability to receive Peakon in payment.
        //
        init(
            sellerItemProvider: Capability<&RockPeaksClipCard.Collection{NonFungibleToken.Provider, RockPeaksClipCard.RockPeaksClipCardCollectionPublic}>,
            itemID: UInt64,
            nodeID: String,
            sellerPaymentReceiver: Capability<&Peakon.Vault{FungibleToken.Receiver}>,
            price: UFix64
        ) {
            pre {
            ...
            assert(
                collectionRef.borrowRockPeaksClipCard(id: itemID) != nil,
    ...
    pub resource interface CollectionManager {
        pub fun insert(offer: @RockPeaksClipCardMarket.SaleOffer)
    ...
    pub resource Collection : CollectionManager, CollectionPurchaser, CollectionPublic {
        ...
        //
         pub fun insert(offer: @RockPeaksClipCardMarket.SaleOffer) {
            ...
            let nodeID: String = offer.nodeID
        ...
        // purchase
        // If the caller passes a valid itemID and the item is still for sale, and passes a Peakon vault
        // typed as a FungibleToken.Vault (Peakon.deposit() handles the type safety of this)
        // containing the correct payment amount, this will transfer the RockPeaksClipCard to the caller's
        // RockPeaksClipCard collection.
        ...
        //   2. RockPeaksClipCard.Withdraw
        //   3. RockPeaksClipCard.Deposit
